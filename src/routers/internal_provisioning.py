...
async def provision_email_outreach(
    ...
    provider = _get_provider_for_capability(capability["id"], "smartlead")
    entitlement = _get_or_create_entitlement(company["org_id"], company_id, capability["id"], provider["id"])

    api_key = _get_org_smartlead_api_key(company["org_id"])
    ...
    provider_config.update(
        {
            "smartlead_client_id": data.smartlead_client_id,
            "provisioning_state": "connected" if data.smartlead_client_id else "pending_client_mapping",
    ...
    updated = supabase.table("company_entitlements").update(
        {
            "status": "connected" if data.smartlead_client_id else "entitled",
            "provider_config": provider_config,
            "updated_at": _now_iso(),
        }
    ).eq("id", entitlement["id"]).eq("org_id", company["org_id"]).execute()
...
async def get_email_outreach_provisioning_status(
    ...
    return _to_response(entitlement.data[0], provider["slug"])
...
async def sync_email_outreach_inboxes(
    ...
    for account in accounts:
        ...
        if account_client_id is not None and str(account_client_id) != str(smartlead_client_id):
            skipped_count += 1
            continue
    ...
    return InboxSyncResponse(
        ...
        smartlead_client_id=int(smartlead_client_id),
